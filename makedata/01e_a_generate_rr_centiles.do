...
global dir "/Users/elizabeth/Dropbox/child SIPP longterm" // ellie's computer //
global results "$dir/analysis/output/JobLosers_SafetyNet"
global joblessnessdir "$dir/analysis/samples"
global samples "$dir/analysis/samples/JobLosers_SafetyNet"

**** read and format IRS data ****
clear
import excel "$joblessnessdir/LMS-UI-data.xlsx", sheet(Imputations) cellrange(A5:F2204)
...
save "$joblessnessdir/IRS_centile_ui.dta", replace
...
use "$samples/sipp_cleaned", clear // making centiles directly off of cleaned analysis data
keep if age>=16
...
save "$joblessnessdir/temp.dta", replace
...
outsheet incid inc_centile year using "$joblessnessdir/SIPP_centiles.csv", comma replace

* dta
import delimited "$joblessnessdir/SIPP_centiles.csv", clear 
save "$joblessnessdir/SIPP_centiles.dta", replace 

*==============================*
**** Create reporting rates ****
use "$joblessnessdir/temp.dta", clear
...
joinby year inc_centile using "$joblessnessdir/IRS_centile_ui.dta", unmatched(master) _merge(IRS_cent_merge)
...
outsheet inc_centile year sipp_ui_recipients IRS_ui_recipients sipp_part_reporting using "$results/SIPP_respondent_reporting.csv", comma replace
...
* save csv of reporting rate
outsheet inc_centile year sipp_ui_dollars IRS_ui_dollars sipp_dollars_reporting using "$results/SIPP_dollars_reporting.csv", comma replace

**** Make dta's ****

* participation 
import delimited "$results/SIPP_respondent_reporting.csv", clear 
save "$results/SIPP_respondent_reporting.dta", replace 

* dollars 
import delimited "$results/SIPP_dollars_reporting.csv", clear 
save "$results/SIPP_dollars_reporting.dta", replace 
...
**** create a dummy file with income bins ****
clear
set obs 501
gen incunemp_bin = 100*(_n - 1)
gen asecwt = 1
...
**** Set up the SIPP data ****
use "$samples/sipp_cleaned", clear
keep if age>=16
...
foreach yr in 1995 1996 1997 1998 1999 2000 2001 2002 2003 2004 2005 2006 2007 2008 2009 2010 2011 2012 2013 {
	...
	*Determine how many people and recipients in each centile to keep only one record that a person in a household received UI that year
	sort incid year 
	by incid year: gen record_count = _n
	keep if record_count == 1

	*centiles on the only 1 obs for each sample unit person 
	local centiles = 10
	gen rand1 = runiform()
	sort year split_tu_totalinc rand1
	by year: gen double runningwgt = sum(p5wgt)
...
* also, save inc_centiles by incid so we can use the same centiles everywhere
collapse anyui, by(incid inc_centile year)
outsheet incid inc_centile year using "$joblessnessdir/SIPP_deciles.csv", comma replace

* dta
import delimited "$joblessnessdir/SIPP_deciles.csv", clear 
save "$joblessnessdir/SIPP_deciles.dta", replace 
