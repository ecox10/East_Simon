...
use "$samples/sipp_cleaned.dta", clear
	...
	** Spousal and Own Earnings and Labor Supply in 1st Survey Month
	...
	sum hinc hearn hpov earn work uhours hinc_sm0 hearn_sm0 hpov_sm0 earn_sm0 work_sm0 uhours_sm0
		
	
	
	
	* Merge in Rothstein and Valletta UI Benefit Weeks data for 2000-2014
	rename statefip fips
	rename imonth month
	*merge m:1 fips year month using "$rv_outputdata/Rothstein_Valetta_UI_Weeks_Calc/eui_state_08-14.dta", force
	...
	*** Months since involuntary job loss: focus on 1st job loss only
		*Des reminder to self on order var 11/16 : "	bysort suid pnum sex: gen order=_n"
	...
	*** Start and End Dates of Job
     ...
     *DES notes: So I believe this is 2015 inflation adjusted CNE: correct
...
replace wic_rr_p=0.846 if year==2014
 ...
 // authors calculations
	...
	* prep imputation
	joinby incid year using "$results/ui_imputed_collapsed.dta", unmatched(master) _merge(impute_merge)
	 
	foreach x in incunemp_impute  {
		gen temp=`x' if wave==1 & refmth==1 
		bysort uniqueid: egen `x'_sm0=max(temp)
		drop temp
	} 
...
*Impute spouses earnings here...


*need to code married at start of ?
	        ********version of spouse_earn that is only for married is below



sum errp_sm0			
					
*foreach var in  earn work { /* whynotwork   uhours ws1occ ws1ind ws2occ ws2ind  numbus wkslok union_yn whystop1 whystop2 loss yr_loss mth_loss num_losses white black hisp other pnum*/
		gen spouse_earn_f =.
		...
		replace max_spouse_earn=. if max_spouse_earn!=max_spouse_earn2 /*there are a handful
		of cases where ref person has both a spouse AND an unmarried partner living with them, replace to be missing */
		drop max_spouse_earn1 max_spouse_earn2
		replace spouse_earn_f =max_spouse_earn if errp_sm0==1 | errp_sm0==2 /*DES note: spouses earnings if job looser is the reference person */
...
*Now code insurance values for adults in household
...
*the missings come from having zero kids at the start of the panel, about 1 million obs.
...
sum anyA_* noA_* numA_* portA0_* portA_*
...
drop _merge
	...
	*browse uniqueid wave refmth counter married min_married_counter min_nmarried_counter
	...
	* Construct Sample of Job Losers w at least 1 year of job tenure 
	...
	tab tenure_1year, m
	tab tenure_6mo, m
	tab tenure_6mo tenure_1year, m
	

	sort uniqueid month_reljl
	* browse uniqueid date_start date_end tenure tenure_1year whystop1 month_reljl loss jobloss tenure_1year_m0
	drop tenure_1year 
	...
	sum earn age female hisp white black other lesshs hs somecol college married kids_m  if tenure_1year==1 & head_spouse_partner==1 & wave==1 & refmth==1 & loser==1 [aw=p5wgt]
	sum earn age female hisp white black other lesshs hs somecol college married kids_m  if head_spouse_partner==1 & wave==1 & refmth==1 & loser==1 [aw=p5wgt]
	sum earn age female hisp white black other lesshs hs somecol college married kids_m if head_spouse_partner==1 & wave==1 & refmth==1 & loser==0 [aw=p5wgt]
	sum earn age female hisp white black other lesshs hs somecol college married kids_m  if head_spouse_partner==1 & wave==1 & refmth==1  [aw=p5wgt]
	sum earn age female hisp white black other lesshs hs somecol college married kids_m  if head_spouse_partner==1 & wave==1 & refmth==1 &  tenure_1year2==1 & age>=24 & age<=60 [aw=p5wgt]
...
***TABLE A1 REPLICATION ALL IN ONE SPOT*******

*do a new log file that just outputs summary stats
cap log close
...
log using "$outputlog/01_CleanData_part2_`today'.log", replace
	...
	*drop dumspell6 sim_spell6		// drop two months before job loss, so everything becomes relative to that
	cap drop jobloss
...
*quickly recode getsbrnt "getting subsidy" for regressions
...
*both relatively small, consider pooling together, but leave seperate for now. 
gen d_housing = (pubhsing==1 | getsbrnt==1)
//replace d_housing = . if pubhsing==. | getsbrnt==. want to count missing as 0s
...
// now only missing a handful for private insurance and missing MANY for FRP variables
mdesc work earn uiamt priv_hins any_hins pub_hins ss_amt  h_ss_amt ssi_amt  ssi_st_amt h_ssi_amt fs_amt tanf_amt ///
wic_amt frp_lunch  hinc hearn hpov100 hpov200 hpov400


// tab outcome variables 6 months prior to jobloss to use as baseline means
*sum work earn spouse_earn marsp_earn uiamt priv_hins any_hins pub_hins ss_amt  h_ss_amt ssi_amt  ssi_st_amt h_ssi_amt fs_amt tanf_amt ///
*wic_amt frp_lunch frp_breakf hinc hearn hpov100 hpov200 hpov400 if tenure_1year==1 & month_reljl==-6 [aw=p5wgt_m0]
...
// because we have already inflation adjsuted the income variables AND already adjusted poverty thresholds 
*gen hpov2015=hpov*237.017/cpi
...
sum hpov_e100 hpov_e200 hpov_e400 hpov100 hpov200 hpov400 hpov_t100 hpov_t200 hpov_t400
...
** HH Inc/Pov Ratio (since these vairables consturcted at the top of the do file, neither of them are cpi adjusted)
...
tabstat earn_quintiles1, by(hinc_quintiles)
tabstat hinc_quintiles1, by(earn_quintiles)
tabstat hpov_sm0_100, by(earn_quintiles)
tabstat hpov_sm0_100200, by(earn_quintiles)
tabstat hpov_sm0_800, by(earn_quintiles)
...
rename balance_pre12post24 balance_1224
	...
	sum age female black college married kids_m0 uiamt any_hins priv_hins pub_hins [aw=p5wgt_m0]
